...
public class UserService implements UserDetailsService/* , Comparator<User> */ {
	...
	public List<User> findAllByOrderByUsername() {
		return usrRepo.findByDeleteFlagOrderByUsername(DeleteFlagConstant.NOT_DELETED);
	}
//
...
//
	...
	public List<User> sarchUser(UserForm userForm, String[] userArray) {
		...
		for (int i = 0; i < userArray.length; i++) {
			sarchUsername = usrRepo.findByUsernameLikeAndDeleteFlag("%" + userArray[i] + "%",DeleteFlagConstant.NOT_DELETED);
			sarchLastName = usrRepo.findByLastNameLikeAndDeleteFlag("%" + userArray[i] + "%",DeleteFlagConstant.NOT_DELETED);
			sarchFirstName = usrRepo.findByFirstNameLikeAndDeleteFlag("%" + userArray[i] + "%",DeleteFlagConstant.NOT_DELETED);
